...
function Add-AccountToEDWAdmin($accountName, $domain, $connString) {
    ...
    elseif (Test-IsGroup -samAccountName $samAccountName -domain $domain){
        Write-DosMessage -Level "Information" -Message "$samAccountName is a group and will not be added as a legacy EDW Admin."
    }
    else {
...
function Add-AccessControlClientRegistration
{
    ...
    $body = @{
        ...
        requireConsent = "false";
        allowedScopes = "openid", "profile", "fabric.profile", "fabric/authorization.read", "fabric/authorization.write", "fabric/authorization.internal", "fabric/identity.searchusers", "fabric/idprovider.searchusers", "fabric/authorization.dos.write";
