...
clientresponse=$(curl -k -X POST -H "Content-Type: application/json" -H "Authorization: Bearer $accesstoken" -d "{ \"clientId\": \"fabric-access-control\", \"clientName\": \"Fabric Authorization Access Control\", \"requireConsent\": false, \"allowedGrantTypes\": [\"implicit\"], \"redirectUris\": [\"$authorizationbaseurl/oidc-callback.html\", \"$authorizationbaseurl/silent.html\"], \"postLogoutRedirectUris\": [ \"$authorizationbaseurl/logged-out\"], \"allowOfflineAccess\": false, \"allowAccessTokensViaBrowser\": true, \"allowedCorsOrigins\":[\"$authorizationbaseurl\"], \"requireConsent\": false, \"allowedScopes\": [\"openid\", \"profile\", \"fabric.profile\", \"fabric/authorization.read\", \"fabric/authorization.write\", \"fabric/idprovider.searchusers\", \"fabric/authorization.dos.write\"]}" $identitybaseurl/api/client)
echo $clientresponse
echo ""
