...
clientresponse=$(curl -v -k -X POST -H "Content-Type: application/json" -H "Authorization: Bearer $accesstoken" -d "{ \"clientId\": \"fabric-access-control\", \"clientName\": \"Fabric Authorization Access Control\", \"requireConsent\": false, \"allowedGrantTypes\": [\"implicit\"], \"redirectUris\": [\"$authorizationbaseurl/client/oidc-callback.html\", \"$authorizationbaseurl/client/silent.html\"], \"postLogoutRedirectUris\": [ \"$authorizationbaseurl/client/logged-out\"], \"allowOfflineAccess\": false, \"allowAccessTokensViaBrowser\": true, \"allowedCorsOrigins\":[\"$authorizationbaseurl\"], \"requireConsent\": false, \"allowedScopes\": [\"openid\", \"profile\", \"fabric.profile\", \"fabric/authorization.read\", \"fabric/authorization.write\", \"fabric/authorization.internal\",\"fabric/idprovider.searchusers\", \"fabric/authorization.dos.write\"]}" $identitybaseurl/api/client)
echo $clientresponse
echo ""
